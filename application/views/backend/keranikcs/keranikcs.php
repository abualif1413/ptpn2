        ...
        <div class="card">
          <div class="card-header">
            <h6 class="text-uppercase mb-0">
                ...
                <label class="pull-right">Data Kerani KCS</label>
            </h6>
          </div>
          <div class="card-body">
...
<script type="text/javascript">
    ...
    $('#btnAdd').click(function () {
        $('#myModal').modal('show');
        $('#myForm').attr('action','<?php echo site_url('Keranikcs/add_keranikcs')?>');
        $('#myModal').find('.modal-title').text('Tambah Data Kerani KCS');
        $('input[name=id]').val('');
    ...
    $(document).ready(function() {
        $("#DataTable_users").DataTable({
            ...
            ajax: {
                url: '<?php echo base_url('Keranikcs/Data_Keranikcs');?>',
                type: 'POST',
  ...
  function Update(id){
    $("#afdeling").empty();
      ...
      $('#btnSave').html('Update');
      $('#myModal').modal('show');
      $('#myForm').attr('action','<?php echo site_url('Keranikcs/update_keranikcs')?>');
      $('#myModal').find('.modal-title').text('Ubah Data');
      $.ajax({
          type : 'GET',
          async: false,
          url  : '<?php echo site_url('Keranikcs/edit_keranikcs')?>',
          data:{
            token:token,
  ...
  function Delete(id) {
    ...
    $.ajax({
        type : 'ajax',
        method :'get',
        url  :'<?php echo site_url('Keranikcs/delete_keranikcs')?>',
        data:{
          token:token,
          '<?php echo $this->security->get_csrf_token_name();?>':'<?php echo $this->security->get_csrf_hash();?>'
        },
...
function setelah_pilih_kebun(id) {
	...
	$.ajax({
		async: false,
		type: "POST",
		url: "<?php echo site_url('Mandor/keraniAskep')?>",
		data: {id_kebun:id},
		cache: false,
		success: function(html)
		{
