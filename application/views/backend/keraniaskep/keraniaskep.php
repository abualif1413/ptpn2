        ...
        <div class="card">
          <div class="card-header">
            <h6 class="text-uppercase mb-0">
                ...
                <label class="pull-right">Data Kerani Askep</label>
            </h6>
          </div>
          <div class="card-body">
...
<script type="text/javascript">
    ...
    $('#btnAdd').click(function () {
        $('#myModal').modal('show');
        $('#myForm').attr('action','<?php echo site_url('Keraniaskep/add_keraniaskep')?>');
    ...
    $(document).ready(function() {
        $("#DataTable_users").DataTable({
            ...
            ajax: {
                url: '<?php echo base_url('Keraniaskep/Data_Keraniaskep');?>',
                type: 'POST',
  ...
  function Update(id){
    $("#afdeling").empty();
      ...
      $('#btnSave').html('Update');
      $('#myModal').modal('show');
      $('#myForm').attr('action','<?php echo site_url('Keraniaskep/update_keraniaskep')?>');
      $('#myModal').find('.modal-title').text('Ubah Data');
      $.ajax({
          type : 'GET',
          async: false,
          url  : '<?php echo site_url('Keraniaskep/Edit_Keraniaskep')?>',
          data:{
            token:token,
  ...
  function Delete(id) {
    ...
    $.ajax({
        type : 'ajax',
        method :'get',
        url  :'<?php echo site_url('Keraniaskep/Delete_Keraniaskep')?>',
        data:{
          token:token,
          '<?php echo $this->security->get_csrf_token_name();?>':'<?php echo $this->security->get_csrf_hash();?>'
        },
