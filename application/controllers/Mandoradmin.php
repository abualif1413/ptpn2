...
class Mandoradmin extends MY_Controller {
	...
	{  
        if ($this->session->userdata('email')) {
            ...
            $data['view_content']   = 'backend/mandoradmin/mandoradmin';
    ...
    {   
        if(self::isPost()){
            ...
            $result = $this->M_mandor->findDataTable($orderBy,$search,$select,false,$join);
            ...
            return $this->M_mandor->findDataTableOutput($data,$search,$select,false,$join);
        }
    }
	...
	public function keraniKCS() {
		$this->load->database();
		$ds_kcs = $this->db->query("SELECT id, nama_lengkap FROM tbl_kerani_kcs WHERE id_afdeling = '" . $this->input->post("id_afdeling") . "' ORDER BY nama_lengkap ASC");
		echo "<option value=''>Pilih</option>";
		foreach ($ds_kcs->result() as $key) {
