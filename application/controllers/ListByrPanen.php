...
class ListByrPanen extends MY_Controller {
	...
	{   
        if ($this->session->userdata('email')) {
            ...
            $data['view_content'] 	= 'backend/listbyrpanen/listbyrpanen';
    ...
    public function datalistbayarpanen_keraniaskep()
    {   
        if($this->isPost()){
            ...
            $select = array(
                "tbl_panen.id",
                "tbl_panen.id_kerani_askep",
                "tbl_kerani_kcs.nama_lengkap as mandor",
                "tbl_pemanen.nama_pemanen",
                "tbl_kebun.nama_kebun",
                "tbl_afdeling.nama_afdeling",
            ...
            $join = array(
                ...
                // array('tbl_komidel','tbl_komidel.kode=tbl_panen.kode'),
            ...
            if($from==''||$to==''){
                $where=array('tbl_panen.approve'=>'N','tbl_panen.id_kerani_askep'=>$id_kerani_askep);
            }else{
                $where=array(
                    'tbl_panen.tanggal >='=>$this->input->post('from'),
                    'tbl_panen.tanggal <='=>$this->input->post('to'),
                    'tbl_panen.id_afdeling'=>$this->input->post('afdeling'),
                    'tbl_panen.approve'=>'N',
                    'tbl_panen.id_kerani_askep'=>$id_kerani_askep,
                );
            }
            // $where=array('tbl_panen.approve'=>'N','tbl_panen.id_kerani_askep'=>$id_kerani_askep);
            ...
            foreach ($result as $item) {
                ...
                $item->totalall=$premi + $item->premi_alat + $brondolan;
                $item->bt=$bt;
                
                if(!$item->nilai_komidel==0){
                    $btnAction = '<button class="btn btn-danger btn-outline-danger btn-mini"  onClick="Validasi(this)" 
                    data-id="'.$item->id.'" 
                    data-id_kerani_kcs="'.$item->id_kerani_kcs.'" 
                    data-id_kebun="'.$item->id_kebun.'" 
                    data-id_afdeling="'.$item->id_afdeling.'"
                    data-id_pemanen="'.$item->id_pemanen.'"
                    data-tph="'.$item->tph.'"
                    data-blok="'.$item->id_blok.'"
                    data-jmlh_panen="'.$item->janjang.'"
                    data-nilai_komidel="'.$item->nilai_komidel.'"
                    data-prestasi="'.$item->kg_fix_byr.'"
                    data-jmlh_brondolan="'.$item->jmlh_brondolan.'"
                    data-bron_tim="'.$item->brondolan_timbang.'"
                    data-koef="'.$item->koef.'"
                    data-tanggal="'.$item->tanggal.'"
                    data-nama_alat="'.$item->nama_alat.'"
                    data-tbs="'.$premi.'"
                    data-premi_alat="'.$item->premi_alat.'"
                    data-premi_brondolan="'.$brondolan.'"
                    data-id_kerani_askep="'.$id_kerani_askep.'"
                    data-total="'.$item->totalall.'"><i class="fa fa-pencil-square-o"></i>Validasi</button>';
                }else{
                     $btnAction = '<button class="btn btn-warning btn-outline-warning btn-mini"><i class="fa fa-pencil-square-o"></i>Nilai Komidel <br>Belum Di Validasi  <br>Kerani Askep</button>';
                }
    ...
    public function validasipanen()
    {
        if(self::isPost()){
            $data = self::validations(array(
                ...
                array('field'=>'bron_tim','label'=>'Brondolan Timbang','rules'=>'required'),
                array('field'=>'koef','label'=>'Pengali Proporsi','rules'=>'required'),
            ...
            if($data === true){
                $field= array(
                    ...
                    'bron_tim'=>$this->input->post('bron_tim'),
