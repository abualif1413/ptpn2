...
class DataHasilPanen020 extends MY_Controller {
	...
	{   
        ...
        if ($this->session->userdata('email')) {
            ...
            $data['view_content'] 	= 'backend/datahasilpanen020/datahasilpanen020';
            $data['token_kcs']      = $this->session->token;
            $data["dataPanen"]      = array();
            $data["tanggal"]        = $this->input->get("tanggal");
            if($this->input->get("tanggal") != "") {
                $this->load->database();
                $sql = "
                    SELECT
                        pemanen.id AS id_pemanen, panen.id AS id_panen, pemanen.nama_pemanen,
                        blok.blok, blok.tahun_tanam, panen.jmlh_panen, panen.jmlh_brondolan,
                        alat.nama_alat
                    FROM
                        tbl_pemanen pemanen
                        LEFT JOIN tbl_kerani_kcs kcs ON pemanen.id_kerani_kcs = kcs.token
                        LEFT JOIN tbl_panen panen ON pemanen.id = panen.id_pemanen AND panen.tanggal = '" . $this->input->get("tanggal") . "'
                        LEFT JOIN tbl_blok blok ON panen.blok = blok.id
                        LEFT JOIN tbl_alat alat ON panen.id_alat = alat.id
                    WHERE
                        kcs.token = '" . $this->session->token . "'
                    ORDER BY
                ...
                foreach($ds->result() as $res) {
                    $temp = array(
                        "id_pemanen"        => $res->id_pemanen,
                        "id_panen"          => $res->id_panen,
                        "nama_pemanen"      => $res->nama_pemanen,
                        "blok"              => $res->blok,
                        "tahun_tanam"       => $res->tahun_tanam,
                        "jmlh_panen"        => $res->jmlh_panen,
                        "jmlh_brondolan"    => $res->jmlh_brondolan,
                        "nama_alat"         => $res->nama_alat
                    );
                    array_push($dataPanen, $temp);
                }
                $this->db->close();
                $data["dataPanen"] = $dataPanen;
            }
